...
\begin{figure}[h]\centering{%
	\begin{tikzpicture}
		...
		\draw (x1) -- (B);
		%\draw (C) -- (B);
		\path [line] (C) -- node [text width=2.5cm,midway] {$C_{2,3}\psi_0$, 2, 1} (B);		
		%\draw (D) -- (A);
		\path [line] (D) -- node [text width=2.5cm,midway, left=1em] {$D_0\psi_0$, 0, 1} (A);
		\path [line] (B) -- node [text width=2.5cm,midway, right=1em] {$B_{1,2}C_{2,3}\psi_0$, 2, 2} (A);
		%\draw (B) -- (A);
...
\caption{Representation of the previous figure's quantum circuit in Genetic Expression Programming.
	The final output is $A_{0,2}B_{1,2}C_{2,3}\psi_0$.}
\end{figure}

The 4-bit quantum circuit in Fig.~\ref{fig:circuit} needs to be represented in genetic expression programming.
Let us first define the Hilbert space $\mathcal{H}=\mathbb{C}^{2^N}$, with its usual inner product.
We also define the set of naturals $I_N=\{0, 1, \cdots, N - 1\}$. We are now ready to define the space
of inputs as $\mathcal{H}\otimes I_N\otimes\mathbb{N}$.

\subsection{One-bit Gates}
If $D$ is a one-bit gate, then it takes one and only one input $(\psi, i, c)\in \mathcal{H}\otimes I_N\otimes\mathbb{N}$ and produces one output
$(D\psi, i, c + 1)$.
